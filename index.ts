...
import { initializeDeploy } from "./src/deploy";
...
export class PumpfunVbot {
    ...
    for (let i = 0; i < chunkedKeypairs.length; i++) {
      ...
      const signers = keypairsInChunk.filter(kp =>
        instructions.some(ix =>
          ix.keys.some((k: { isSigner: any; pubkey: { equals: (arg0: any) => any; }; }) => k.isSigner && k.pubkey.equals(kp.publicKey))
        )
      );
  ...
  async extendLUT() {
    try {
      ...
      const existingAddresses = new Set(this.lookupTableAccount.state.addresses.map((addr: { toBase58: () => any; }) => addr.toBase58()));
    ...
    try {
      ...
      for (let i = 0; i < chunkedKeypairs.length; i++) {
        ...
        keypairsInChunk.forEach(kp => {
          if (instructions.some(ix => ix.keys.some((k: { isSigner: any; pubkey: { equals: (arg0: any) => any; }; }) => k.isSigner && k.pubkey.equals(kp.publicKey)))) {
